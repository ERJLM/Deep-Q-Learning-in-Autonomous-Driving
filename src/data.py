...
class BDD10K(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        if i >= len(self.images) or i >= len(self.masks):
            raise IndexError(f"Index {i} out of range. Dataset size: {len(self.images)}")
...
class BDD10K_Binary(BDD10K):
    ...
    def __getitem__(self, i):
        ...
        d['mask'] = (d['mask'] >= 6) #.astype(torch.uint8)
        return d

if __name__ == '__main__':  # debug
    import matplotlib.pyplot as plt
    ds = BDD10K('/Users/eliandromelo/Downloads', 'train')
    d = ds[3]
    plt.subplot(1, 2, 1)
    plt.imshow(d['image'])
    plt.subplot(1, 2, 2)
